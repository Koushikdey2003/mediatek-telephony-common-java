...
class TcpServerChannel extends Channel {
    ...
    public int openChannel(BipCmdMessage cmdMsg, Network network) {
        ...
        if (this.mChannelStatus == 0 || this.mChannelStatus == 2) {
            ...
            Thread thread = new Thread(new Channel.UICCServerThread(this));
            this.rt = thread;
            thread.start();
        }
    ...
    public int closeChannel() {
        ...
        if (true == this.mCloseBackToTcpListen) {
            if (-128 == this.mChannelStatusData.mChannelStatus) {
                try {
                    try {
                        ...
                        BufferedOutputStream bufferedOutputStream = this.mOutput;
            ...
            try {
                try {
                    DataInputStream dataInputStream2 = this.mInput;
                    if (dataInputStream2 != null) {
                        dataInputStream2.close();
                    }
                    BufferedOutputStream bufferedOutputStream2 = this.mOutput;
                    ...
                    ServerSocket serverSocket = this.mSSocket;
