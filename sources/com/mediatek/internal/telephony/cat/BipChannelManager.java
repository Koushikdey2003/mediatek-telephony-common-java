...
class BipChannelManager {
    ...
    public void releaseChannelId(int cId, int protocolType) {
        ...
        try {
            ...
            if (3 == protocolType && (b2 = this.mCurrentOccupiedUICCSerCh) >= 0) {
                this.mCurrentOccupiedUICCSerCh = (byte) (b2 - 1);
            } else if ((1 != protocolType && 2 != protocolType) || (b = this.mCurrentOccupiedPSCh) < 0) {
                MtkCatLog.e("[BIP]", "releaseChannelId, bad parameters.cId:" + cId + ":" + ((int) this.mChannelIdPool));
            } else {
                this.mCurrentOccupiedPSCh = (byte) (b - 1);
            }
    ...
    public void releaseChannelId(int cId) {
        ...
        try {
            ...
            if (channelArr[cId - 1] == null) {
                MtkCatLog.e("[BIP]", "channel object is null.");
                return;
            }
            int protocolType = channelArr[cId - 1].mProtocolType;
            if (3 == protocolType && (b2 = this.mCurrentOccupiedUICCSerCh) > 0) {
                this.mCurrentOccupiedUICCSerCh = (byte) (b2 - 1);
            } else if ((1 != protocolType && 2 != protocolType) || (b = this.mCurrentOccupiedPSCh) <= 0) {
                MtkCatLog.e("[BIP]", "releaseChannelId, bad parameters.cId:" + cId + ":" + ((int) this.mChannelIdPool));
    ...
    public boolean isClientChannelOpened() {
        ...
        for (int i = 0; i < 7; i++) {
            try {
                Channel[] channelArr = this.mChannels;
                if (channelArr != null && (channel = channelArr[i]) != null && (channel.mProtocolType & 3) != 0) {
                    return true;
                }
