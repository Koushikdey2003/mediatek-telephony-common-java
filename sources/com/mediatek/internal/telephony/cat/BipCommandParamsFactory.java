...
class BipCommandParamsFactory extends Handler {
    ...
    static synchronized BipCommandParamsFactory getInstance(BipRilMessageDecoder caller, IccFileHandler fh, Context context) {
        BipCommandParamsFactory bipCommandParamsFactory = sInstance;
        if (bipCommandParamsFactory != null) {
            return bipCommandParamsFactory;
        }
        if (fh == null) {
            return null;
        }
        return new BipCommandParamsFactory(caller, fh, context);
    }

    private BipCommandParamsFactory(BipRilMessageDecoder caller, IccFileHandler fh, Context context) {
        this.mCaller = null;
        this.mNoAlphaUsrCnf = false;
        this.mCaller = caller;
        this.mIconLoader = BipIconLoader.getInstance(this, fh, caller.getSlotId());
        try {
            this.mNoAlphaUsrCnf = context.getResources().getBoolean(R.bool.config_mms_content_disposition_support);
    ...
    /* JADX INFO: renamed from: com.mediatek.internal.telephony.cat.BipCommandParamsFactory$1, reason: invalid class name */
    static /* synthetic */ class AnonymousClass1 {
        ...
        static {
            ...
            try {
                iArr[AppInterface.CommandType.SET_UP_EVENT_LIST.ordinal()] = 1;
    ...
    private boolean processOpenChannel(CommandDetails cmdDet, List<ComprehensionTlv> ctlvs) throws ResultException {
        ...
        if (ctlv == null) {
            ...
            confirmText = confirmText3;
            confirmText.text = MtkValueParser.retrieveAlphaId(ctlv, this.mNoAlphaUsrCnf);
        }
        ...
        if (ctlv3 == null) {
            ...
            if (bearerDesc3 instanceof GPRSBearerDesc) {
                ...
                accessPointName = null;
                MtkCatLog.d("[BIP]", "\ntrafficClass: " + ((UTranBearerDesc) bearerDesc3).trafficClass + "\nmaxBitRateUL_High: " + ((UTranBearerDesc) bearerDesc3).maxBitRateUL_High + "\nmaxBitRateUL_Low: " + ((UTranBearerDesc) bearerDesc3).maxBitRateUL_Low + "\nmaxBitRateDL_High: " + ((UTranBearerDesc) bearerDesc3).maxBitRateDL_High + "\nmaxBitRateUL_Low: " + ((UTranBearerDesc) bearerDesc3).maxBitRateDL_Low + "\nguarBitRateUL_High: " + ((UTranBearerDesc) bearerDesc3).guarBitRateUL_High + "\nguarBitRateUL_Low: " + ((UTranBearerDesc) bearerDesc3).guarBitRateUL_Low + "\nguarBitRateDL_High: " + ((UTranBearerDesc) bearerDesc3).guarBitRateDL_High + "\nguarBitRateDL_Low: " + ((UTranBearerDesc) bearerDesc3).guarBitRateDL_Low + "\ndeliveryOrder: " + ((UTranBearerDesc) bearerDesc3).deliveryOrder + "\nmaxSduSize: " + ((UTranBearerDesc) bearerDesc3).maxSduSize + "\nsduErrorRatio: " + ((UTranBearerDesc) bearerDesc3).sduErrorRatio + "\nresidualBitErrorRadio: " + ((UTranBearerDesc) bearerDesc3).residualBitErrorRadio + "\ndeliveryOfErroneousSdus: " + ((UTranBearerDesc) bearerDesc3).deliveryOfErroneousSdus + "\ntransferDelay: " + ((UTranBearerDesc) bearerDesc3).transferDelay + "\ntrafficHandlingPriority: " + ((UTranBearerDesc) bearerDesc3).trafficHandlingPriority + "\npdp type: " + ((UTranBearerDesc) bearerDesc3).pdpType);
            } else {
                accessPointName = null;
        ...
        if (ctlv8 != null) {
            ...
            transportProtocol2 = BipValueParser.retrieveTransportProtocol(ctlv8);
            MtkCatLog.d("[BIP]", "CPF-processOpenChannel: transport protocol(type/port): " + transportProtocol2.protocolType + "/" + transportProtocol2.portNumber);
            if ((1 == transportProtocol2.protocolType || 2 == transportProtocol2.protocolType) && bearerDesc == null) {
    ...
    private boolean processReceiveData(CommandDetails cmdDet, List<ComprehensionTlv> ctlvs) throws ResultException {
        ...
        if (ctlv2 != null) {
            textMsg.text = MtkValueParser.retrieveAlphaId(ctlv2, this.mNoAlphaUsrCnf);
        }
