...
public class WorldPhoneOm extends Handler implements IWorldPhone {
    ...
    private static int sSimLockedSlotId;
    ...
    private final BroadcastReceiver mWorldPhoneReceiver = new BroadcastReceiver() { // from class: com.mediatek.internal.telephony.worldphone.WorldPhoneOm.1
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals("android.telephony.action.SIM_APPLICATION_STATE_CHANGED")) {
                ...
                WorldPhoneOm.sMajorSim = WorldPhoneUtil.getMajorSim();
            ...
            if (action.equals(IWorldPhone.ACTION_TEST_WORLDPHONE)) {
                ...
                WorldPhoneOm.this.handleWorldPhoneTest(fakeUserType, fakeRegion);
    ...
    private Runnable mTddStandByTimerRunnable = new Runnable() { // from class: com.mediatek.internal.telephony.worldphone.WorldPhoneOm.2
        @Override // java.lang.Runnable
        public void run() {
            WorldPhoneOm.sTddStandByCounter++;
            if (WorldPhoneOm.sTddStandByCounter >= WorldPhoneOm.TDD_STANDBY_TIMER.length) {
                WorldPhoneOm.sTddStandByCounter = WorldPhoneOm.TDD_STANDBY_TIMER.length - 1;
            }
            if (WorldPhoneOm.sBtSapState == 0) {
                WorldPhoneOm.logd("TDD time out!");
                WorldPhoneOm.sIsWaintInTddTimeOut = 1;
    ...
    private Runnable mFddStandByTimerRunnable = new Runnable() { // from class: com.mediatek.internal.telephony.worldphone.WorldPhoneOm.3
        @Override // java.lang.Runnable
        public void run() {
            WorldPhoneOm.sFddStandByCounter++;
            if (WorldPhoneOm.sFddStandByCounter >= WorldPhoneOm.FDD_STANDBY_TIMER.length) {
                WorldPhoneOm.sFddStandByCounter = WorldPhoneOm.FDD_STANDBY_TIMER.length - 1;
            }
            if (WorldPhoneOm.sBtSapState == 0) {
                WorldPhoneOm.logd("FDD time out!");
                WorldPhoneOm.sIsWaintInFddTimeOut = 1;
    ...
    private Runnable mEmsrResumeByTimerRunnable = new Runnable() { // from class: com.mediatek.internal.telephony.worldphone.WorldPhoneOm.4
        @Override // java.lang.Runnable
        public void run() {
            WorldPhoneOm.sWaitInEmsrResume = false;
            WorldPhoneOm.sMajorSim = WorldPhoneUtil.getMajorSim();
    ...
    private BroadcastReceiver mWorldPhoneEccStateReceiver = new BroadcastReceiver() { // from class: com.mediatek.internal.telephony.worldphone.WorldPhoneOm.5
        @Override // android.content.BroadcastReceiver
        public void onReceive(Context context, Intent intent) {
    ...
    static {
        ...
        PLMN_TABLE_TYPE1 = new String[]{"46000", "46002", "46004", "46007", "46008"};
        PLMN_TABLE_TYPE1_EXT = new String[]{"45412"};
        PLMN_TABLE_TYPE3 = new String[]{"46001", "46006", "46009", "45407", "46003", "46005", "45502", "46011"};
        MCC_TABLE_DOMESTIC = new String[]{RadioCapabilitySwitchUtil.CN_MCC};
        sContext = null;
        sDefultPhone = null;
        sProxyPhones = null;
        ...
        sImsi = new String[projectSimNum];
        sDefaultBootuUpModem = 0;
        sSuspendId = new int[projectSimNum];
        sIsInvalidSim = new boolean[projectSimNum];
        sSuspendWaitImsi = new boolean[projectSimNum];
        sFirstSelect = new boolean[projectSimNum];
        sUiccController = null;
        sIccRecordsInstance = new IccRecords[projectSimNum];
    ...
    public WorldPhoneOm() {
        ...
        Phone phone2 = sDefultPhone;
        if (phone2 != null) {
            sContext = phone2.getContext();
        } else {
            logd("DefaultPhone = null");
        }
        sVoiceCapable = sContext.getResources().getBoolean(R.bool.config_setColorTransformAcceleratedPerLayer);
        sContext.registerReceiver(this.mWorldPhoneReceiver, intentFilter, 2);
        ...
        SystemProperties.set(IWorldPhone.WORLD_PHONE_FDD_MODEM_TIMER, Integer.toString(iArr[sFddStandByCounter]));
        logd("FDD_STANDBY_TIMER = " + iArr[sFddStandByCounter] + "s");
    ...
    public void handleSimApplicationStateChanged(int slotId, int state) {
        if (true == WorldPhoneUtil.getSimLockedState(state) && WorldPhoneUtil.getModemSelectionMode() == 1 && (slotId != sMajorSim || slotId == sSimLockedSlotId)) {
        ...
        if (!WorldPhoneUtil.getSimLockedState(state) && WorldPhoneUtil.getModemSelectionMode() == 1 && ((slotId != sMajorSim || slotId == sSimLockedSlotId) && sSimLocked == 1)) {
        ...
        if (state == 10) {
            ...
            sUiccController = uiccController;
            if (uiccController != null) {
                sIccRecordsInstance[slotId] = sProxyPhones[slotId].getIccCard().getIccRecords();
                IccRecords iccRecords = sIccRecordsInstance[slotId];
                if (iccRecords != null) {
                    sImsi[slotId] = iccRecords.getIMSI();
                    if (WorldPhoneUtil.getModemSelectionMode() == 1 && slotId == sMajorSim) {
                        logd("Major SIM");
                        int userType = getUserType(sImsi[slotId]);
                        sUserType = userType;
                        boolean[] zArr = sFirstSelect;
                        if (zArr[slotId]) {
                            zArr[slotId] = false;
                            if (userType == 1) {
                                int i = sRegion;
                        ...
                        boolean[] zArr2 = sSuspendWaitImsi;
                        if (zArr2[slotId]) {
                            zArr2[slotId] = false;
    ...
    private void handlePlmnChange(AsyncResult ar, int slotId) {
        ...
        if (ar.exception == null && ar.result != null) {
            ...
            int i2 = WorldPhoneUtil.getModemSelectionMode();
            if (i2 == 1) {
    ...
    private void handleGmssRatChange(AsyncResult ar, int slotId) {
        ...
        if (ar.exception == null && ar.result != null) {
            ...
            if (WorldPhoneUtil.getModemSelectionMode() == 1) {
                int region = getRegion(mccString);
                sRegion = region;
                if (sUserType != 3 && region == 2 && sMajorSim != -1) {
                    handleSwitchModem(100);
                    return;
                }
                return;
    ...
    private void handleServiceStateChange(AsyncResult ar, int slotId) {
        ...
        if (ar.exception == null && ar.result != null) {
            ...
            if (serviceState != null) {
                ...
                logd("sRilVoiceRadioTech: " + ServiceState.rilRadioTechnologyToString(sRilVoiceRadioTechnology) + ", sDataRegState: " + sDataRegState);
                logd("sRilDataRadioTech: , " + ServiceState.rilRadioTechnologyToString(sRilDataRadioTechnology));
                ModemSwitchHandler.getActiveModemType();
                if (slotId == sMajorSim) {
                    if (WorldPhoneUtil.getModemSelectionMode() == 1) {
    ...
    private void handleRegistrationSuspend(AsyncResult ar, int slotId) {
        ...
        if (ar.exception == null && ar.result != null) {
            ...
            if (WorldPhoneUtil.getModemSelectionMode() == 1 && sMajorSim == slotId) {
                if (sUserType != 0) {
                    resumeCampingProcedure(slotId, true);
                    return;
                }
    ...
    public boolean handleSwitchModem(int toModem) {
        ...
        if (WorldPhoneUtil.getModemSelectionMode() == 1 && WorldPhoneUtil.isCdmaLteDcSupport() && !isNeedSwitchModem(mMajorSim)) {
    ...
    public void handleSimSwitched() {
        ...
        String str = sImsi[sMajorSim];
        if (str == null || str.equals("")) {
            logd("Major slot IMSI not ready");
            sUserType = 0;
            return;
        }
        int userType = getUserType(sImsi[sMajorSim]);
        sUserType = userType;
        if (userType == 1) {
        ...
        if (iccState == IccCardConstants.State.READY) {
            ...
            if (i == 1) {
                if (mdType == 6 || mdType == 4) {
                    int[] iArr = TDD_STANDBY_TIMER;
                    if (iArr[sTddStandByCounter] >= 0) {
                        if (!sWaitInTdd) {
                            sWaitInTdd = true;
                            logd("Wait " + iArr[sTddStandByCounter] + "s. Timer index = " + sTddStandByCounter);
                            postDelayed(this.mTddStandByTimerRunnable, ((long) iArr[sTddStandByCounter]) * 1000);
                        } else {
                            logd("Timer already set:" + iArr[sTddStandByCounter] + "s");
                        }
                    } else {
                        logd("Standby in TDD modem");
                    }
                } else if (mdType == 5 || mdType == 3) {
                    int[] iArr2 = FDD_STANDBY_TIMER;
                    if (iArr2[sFddStandByCounter] >= 0) {
                        if (!sWaitInFdd) {
                            sWaitInFdd = true;
                            logd("Wait " + iArr2[sFddStandByCounter] + "s. Timer index = " + sFddStandByCounter);
                            postDelayed(this.mFddStandByTimerRunnable, ((long) iArr2[sFddStandByCounter]) * 1000);
                        } else {
                            logd("Timer already set:" + iArr2[sFddStandByCounter] + "s");
                        }
                    } else {
    ...
    private boolean isNoService() {
        boolean noService;
        if (sVoiceRegState == 1 && sDataRegState == 1) {
            noService = true;
        } else {
            noService = false;
        }
    ...
    public void notifyRadioCapabilityChange(int capailitySimId) {
        ...
        String str = sImsi[capailitySimId];
        if (str == null || str.equals("")) {
            logd("Capaility slot IMSI not ready");
            sUserType = 0;
            return;
        }
        int userType = getUserType(sImsi[capailitySimId]);
        sUserType = userType;
        if (userType == 1) {
            String[] strArr = sNwPlmnStrings;
            if (strArr != null) {
    ...
    public void handleWorldPhoneTest(int fakeUserType, int fakeRegion) {
        ...
        sMajorSim = majorSim;
        if (majorSim != -99 && majorSim != -1) {
            String imsi = sImsi[majorSim];
            if (imsi != null && !imsi.equals("")) {
                String imsi2 = imsi.substring(0, 5);
                switch (fakeUserType) {
                    case 1:
