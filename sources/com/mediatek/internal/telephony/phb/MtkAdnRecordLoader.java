...
public class MtkAdnRecordLoader extends AdnRecordLoader {
    ...
    public void loadAllFromEF(int ef, int extensionEF, Message response) {
        ...
        int type = getPhbStorageType(ef);
        if (type != -1) {
            this.mFh.mCi.queryPhbStorageInfo(type, obtainMessage(EVENT_PHB_QUERY_STAUTS));
        } else {
        ...
        try {
            switch (msg.what) {
                ...
                case 3:
                    ...
                    ArrayList<MtkAdnRecord> arrayList = new ArrayList<>(datas.size());
                    this.mAdns = arrayList;
                    this.mResult = arrayList;
                    this.mPendingExtLoads = 0;
                    int s = datas.size();
                    for (int i = 0; i < s; i++) {
                        MtkAdnRecord adn3 = new MtkAdnRecord(this.mEf, i + 1, datas.get(i));
                        this.mAdns.add(adn3);
                ...
                case 103:
                    ...
                    break;
                    break;
                case EVENT_PHB_LOAD_ALL_DONE /* 104 */:
                    ...
                    break;
                    break;
                case EVENT_PHB_QUERY_STAUTS /* 105 */:
    ...
    private void readEntryFromModem(int type, int[] readInfo) {
        ...
        this.mFh.mCi.readPhbEntry(type, readInfo[0], eIndex, obtainMessage(EVENT_PHB_LOAD_ALL_DONE, readInfo));
    }
    ...
    private MtkAdnRecord getAdnRecordFromPhbEntry(PhbEntry entry) {
        ...
        try {
            ...
            return new MtkAdnRecord(this.mEf, entry.index, alphaId, number.replace('?', 'N').replace('p', ',').replace('w', ';'));
