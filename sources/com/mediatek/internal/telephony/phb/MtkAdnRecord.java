...
public class MtkAdnRecord extends AdnRecord {
    public static final Parcelable.Creator<MtkAdnRecord> CREATOR = new Parcelable.Creator<MtkAdnRecord>() { // from class: com.mediatek.internal.telephony.phb.MtkAdnRecord.1
    ...
    public String getAdditionalNumber(int index) {
        if (index == 0) {
            String number = this.mAdditionalNumber;
            return number;
        }
        if (index == 1) {
            String number2 = this.mAdditionalNumber2;
            return number2;
        }
        if (index == 2) {
            String number3 = this.mAdditionalNumber3;
            return number3;
        }
    ...
    public String toString() {
        return "ADN Record:" + this.mRecordNumber + ",alphaTag:" + getMaskString(this.mAlphaTag) + ",number:" + getMaskString(this.mNumber) + ",aas:" + this.mAas + ",emails:" + (this.mEmails == null ? "null" : getMaskString(this.mEmails[0])) + ",grpIds:" + this.mGrpIds + ",sne:" + this.mSne;
    }
    ...
    public byte[] buildAdnString(int recordSize) {
        ...
        if (TextUtils.isEmpty(this.mNumber)) {
            ...
            try {
                ...
                CommandException cmdEx = new CommandException(CommandException.Error.OEM_ERROR_12);
    ...
    public void appendExtRecord(byte[] extRecord) {
        try {
            if (extRecord.length == 13 && (extRecord[0] & 3) == 2 && (extRecord[1] & 255) <= 10) {
                this.mNumber += MtkPhoneNumberUtils.calledPartyBCDFragmentToString(extRecord, 2, extRecord[1] & 255);
            }
    ...
    private boolean isPhoneNumberInvaild(String phoneNumber) {
        if (!TextUtils.isEmpty(phoneNumber)) {
            String tempPhoneNumber = MtkPhoneNumberUtils.stripSeparators(phoneNumber);
            if (!Pattern.matches(SIM_NUM_PATTERN, MtkPhoneNumberUtils.extractCLIRPortion(tempPhoneNumber))) {
                return true;
            }
            return false;
        }
